$(document).ready(function() {
    ...
    const table = $('#historyTable').DataTable({
        order: [[1, 'desc']], // Changed from [[2, 'desc']] to [[1, 'desc']] for Start Time column
        pageLength: 25,
        responsive: true
    });
    ...
    // Status filter handler - Fixed to work with badge content
    $('#statusFilter').on('change', function() {
        const selectedStatus = this.value.toLowerCase();
        if (selectedStatus === '') {
            table.column(4).search('').draw(); // Status is column 4
        } else {
            // Search for the status within the badge content
            table.column(4).search(selectedStatus, false, false).draw();
        }
    });
    ...
    $.fn.dataTable.ext.search.push(
        function(settings, data, dataIndex) {
            ...
            const rowDate = new Date(data[1]).setHours(0,0,0,0); // Start Time is column 1
